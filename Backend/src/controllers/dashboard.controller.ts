...
import { getCalendarStats, getRecentPosts } from './aiCalendarController';
...
export class DashboardController {
    static async getStats(req: Request, res: Response) {
        try {
            ...
            const contentStats = getCalendarStats();

            // 2. Lead Stats (from DB)
            const totalLeads = await Lead.countDocuments({ businessId });
            const newLeads = await Lead.countDocuments({ businessId, status: 'new' });
            const qualifiedLeads = await Lead.countDocuments({ businessId, status: 'qualified' });
            const calls = await Lead.countDocuments({ businessId, status: 'contacted' });
            ...
            const totalCalls = await Transcript.countDocuments({ businessId });
            ...
            if (transcriptAgg.length > 0) {
                if (transcriptAgg[0].positiveCount > transcriptAgg[0].negativeCount) sentiment = 'Positive';
                else if (transcriptAgg[0].negativeCount > transcriptAgg[0].positiveCount) sentiment = 'Negative';
            }
            ...
            // 5. Recent Activity
            const recentLeads = await Lead.find({ businessId }).sort({ createdAt: -1 }).limit(5);
            const recentTranscripts = await Transcript.find({ businessId }).sort({ createdAt: -1 }).limit(5);
            const recentPosts = getRecentPosts();

            const events = [
                ...recentLeads.map(l => ({ type: 'lead', data: l, date: l.createdAt })),
                ...recentTranscripts.map(t => ({ type: 'call', data: t, date: t.createdAt })),
                ...recentPosts.map((p: any) => ({ type: 'content', data: p, date: p.savedAt }))
